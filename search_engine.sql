...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 08, 2022 at 04:07 PM
...
(23, 'deansa@gmail.com', 'Dean', 'Elgar', 'reguser', '123', '2022-01-10 14:31:12'),
...
(39, 'mithun@gmail.com', 'Mithun', 'RR', 'user', 'db413d6fbb1d9d0ced3e178e8adbcd97', '2022-06-08 12:45:45');
...
--
-- Stand-in structure for view `scheme_report`
-- (See below for the actual view)
--
CREATE TABLE `scheme_report` (
`scheme_type` varchar(255)
,`no_of_person` bigint(21)
);
...
--
-- Structure for view `scheme_report`
--
DROP TABLE IF EXISTS `scheme_report`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `scheme_report`  AS SELECT `scheme`.`scheme_type` AS `scheme_type`, count(0) AS `no_of_person` FROM ((`reg_user` join `scheme` on(`reg_user`.`scheme_id` = `scheme`.`scheme_id`)) join `scheme_details` on(`scheme`.`scheme_type` = `scheme_details`.`scheme_type`)) GROUP BY `scheme`.`scheme_type` ;
...
--
ALTER TABLE `content`
  ADD CONSTRAINT `content` FOREIGN KEY (`author_id`) REFERENCES `upload_rights` (`account_id`);

--
-- Constraints for table `images`
--
ALTER TABLE `images`
  ADD CONSTRAINT `images` FOREIGN KEY (`author_id`) REFERENCES `upload_rights` (`account_id`);

--
-- Constraints for table `reg_user`
--
ALTER TABLE `reg_user`
  ADD CONSTRAINT `reg_user_ibfk_1` FOREIGN KEY (`account_id`) REFERENCES `account_details` (`account_id`),
  ADD CONSTRAINT `reg_user_ibfk_2` FOREIGN KEY (`scheme_id`) REFERENCES `scheme` (`scheme_id`);

--
-- Constraints for table `renewal_details`
--
ALTER TABLE `renewal_details`
  ADD CONSTRAINT `renewal_details` FOREIGN KEY (`account_id`) REFERENCES `reg_user` (`account_id`);

--
-- Constraints for table `scheme_details`
--
ALTER TABLE `scheme_details`
  ADD CONSTRAINT `scheme_details_ibfk_1` FOREIGN KEY (`scheme_type`) REFERENCES `scheme` (`scheme_type`);

--
-- Constraints for table `search_history`
--
ALTER TABLE `search_history`
  ADD CONSTRAINT `search_history` FOREIGN KEY (`account_id`) REFERENCES `account_details` (`account_id`);

--
-- Constraints for table `videos`
--
ALTER TABLE `videos`
  ADD CONSTRAINT `videos` FOREIGN KEY (`author_id`) REFERENCES `upload_rights` (`account_id`);
COMMIT;
